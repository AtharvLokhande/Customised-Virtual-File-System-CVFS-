...
void DisplayHelp()
{
	...
	printf("fstat : To Display informationof file using file descripton\n");
...
void ls_file()
{
	...
	printf("\nFile Name\tInode number\tFile Actual Size\tFile count\n");
	printf("---------------------------------------------------------------------\n");
	while(temp != NULL)
	{
		if(temp->FileType != 0)
	...
	while(1)
	{
		...
		count = sscanf(str,"%s %s %s %s",command[0],command[1],command[2],command[3]);
			...
			if(count == 1)
			{
				...
				{
					printf("Termminating the Virtual File System\n");
					break;
				}
				else
				{
			...
			else if(count == 2)
			{
				...
				else if(strcmp(command[0],"write")==0)
				{
					...
					if(ret == 0)
					{
						printf("Error : Incorrect prameter\n");
						continue;
					}
					...
					if(ret == -3)
						printf("ERROR : It is not refgular File\n");
				}
